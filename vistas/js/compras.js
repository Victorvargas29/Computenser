...
function listar(){
	tabla=$('#detalles_compras').dataTable({ 
	...
	tabla=$('#sub').dataTable({ 
		...
		"ajax":
		{
			url:'../ajax/compras.php?op=listarSubtotales',
			type: "get",
			datatype: "json",
			error: function(e){
				console.log(e.responseText);
...
function agregar_detalles(){
	...
	jsomdetalle=formData;
	console.log(jsomdetalle);

	/*
		$.ajax({
			url: "../ajax/compras.php?op=agregar_detalle",
			type: "POST",
			data: formData,
			cache:false,
			contentType: false,
			...
			///	console.log(formData); //muestre los valores en la consola
				...
				$('#detalles_compras').DataTable().ajax.reload();
...
function idfactura(){
	var idFf=1;
	$.post("../ajax/compras.php?op=mostrar", function(data, status)
	{	idFf=1;
		console.log(data);			
		data =JSON.parse(data);	 
		idF =data.idFactura;
...
function eliminar_item(iddetallesFT){
	...
	$.ajax({
		url:"../ajax/compras.php?op=eliminar_item",
		method:"POST",
...
function registrar(){
	...
	//console.log("registrarrrrrr");
		$.ajax({
			url: "../ajax/compras.php?op=guardarCompra",
			type: "POST",
			data: formData,
			contentType: false,
			processData: false,
...
function borrar_temporal(){
	var formData = new FormData($("#form_compra")[0]);
	$.ajax({
		url: "../ajax/compras.php?op=borrar_temp",
		type: "POST",
		data: formData,
		contentType: false,
		processData: false,
...
function listarfacturas(){
	...
	tabla=$('#factura_data').dataTable({  //#usuario_data este es el id de la tabla
		...
		"ajax":
		{
			url:'../ajax/compras.php?op=listarfacturas',
			type: "get",
			datatype: "json",
			error: function(e){
				console.log(e.responseText);
...
function tipomoneda(idFactura, tipo_moneda){
	...
	.then(result => {
		 if (result.value) {
			   $.ajax({
				url:"../ajax/compras.php?op=activarydesactivar",
				method:"POST",
...
if(anulado!=1){
	...
	.then(result => {
		 if (result.value) {
			   $.ajax({
				url:"../ajax/compras.php?op=anular",
				method:"POST",
